...
{
  std::shared_ptr<rs2::depth_frame> spDepth;
  UserDatum()
  {}
  ...
  WUserInput()
    {
	  ...
	  mCfg.enable_stream(RS2_STREAM_DEPTH, 640, 480, RS2_FORMAT_Z16, 30);
  ...
  std::shared_ptr<std::vector<UserDatum>> workProducer()
    {
	  try
        {
		  mFrames = mPipe.wait_for_frames();
		  mColor = mFrames.get_color_frame();
		  // Close program when empty frame
		  if (!mColor)
            {
			  op::log("No more frames to read! Closing program after previous frames are processed.",
					  op::Priority::High);
		  ...
		  else
            {
			  ...
			  // Fill datum
			  datum.cvInputData = cv::Mat(cv::Size(640,480), CV_8UC3, (void*)mColor.get_data(), cv::Mat::AUTO_STEP);
			  datum.spDepth = std::make_shared<rs2::depth_frame>(mFrames.get_depth_frame());

			  // If empty frame -> return nullptr
			  if (datum.cvInputData.empty())
                {
				  op::log("Empty frame detected after converting to mat! Closing program.",
						  op::Priority::High);
				  this->stop();
				  datumsPtr = nullptr;
                }
...
{
 public:
  WUserOutput(std::string url = "http://127.0.0.1:1337")
	{
	  if(!mStreamer.connect(url))
		{
		  op::log("Could not connect to socket io server!", op::Priority::High);
		  this->stop();
		}
	}
  void initializationOnThread() {}
	...
	try
	  {
		if(datumsPtr != nullptr && !datumsPtr->empty())
		  {
			std::string distance("\nDistance: " + std::to_string(datumsPtr->at(0).spDepth->get_distance(320,240)));
			op::log(distance);
			mStreamer.sendMessage("log", distance);
